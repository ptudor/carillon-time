...
func TestEngineSourceExitMarksItUnreachableAndRestarts(t *testing.T) {
	...
	if st := e.Status(); st.ClockState != discipline.StateHoldover || st.State != discipline.StateHoldover || !clk.Status().Synced {
		t.Fatalf("a plain client should retain synchronization through holdover: %+v", st)
	}
