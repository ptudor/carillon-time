...
The public repository is `https://github.com/ptudor/carillon-time`.
...
> A pure-Go NTP time daemon for Linux and FreeBSD, with GPS, serial PPS, and inspectable clock discipline.

Suggested topics: `ntp`, `pps`, `gps`, `time-synchronization`, `golang`, `freebsd`, `linux`.
...
```sh
gh attestation verify FILE --repo ptudor/carillon-time --signer-workflow ptudor/carillon-time/.github/workflows/release.yml
```
