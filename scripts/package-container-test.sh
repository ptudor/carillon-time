...
for service in $services; do
    ...
    # Linux comm names stop at 15 bytes.
...
carillon -version
runuser -u carillon -- carillon -check -config /etc/carillon/carillon.toml
# Never run the clock daemon in this test. -check exits before clock.New().
